...
if "_form_counter" not in st.session_state:
...
with c1:
    ...
    if source_choice == "+ 新建来源":
        new_source = st.text_input("输入新来源名称", key=f"new_source_{fc}")
...
with c3:
    ...
    if chapter_choice == "+ 新建章节":
        new_chapter = st.text_input("输入新章节名称", key=f"new_chapter_{fc}")
...
q_img = st.file_uploader(
    ...
    key=f"question_image_upload_{fc}",
    help="直接拍照上传题干"
)
if q_img:
...
question_text = st.text_area(
    "题干补充说明（可选，支持 $LaTeX$）",
    height=80, key=f"question_text_{fc}",
...
s_img = st.file_uploader(
    ...
    key=f"solution_image_upload_{fc}",
    help="拍照上传正确解析过程"
)
if s_img:
...
standard_solution = st.text_area(
    "正解补充说明（可选，支持 $LaTeX$）",
    height=80, key=f"standard_solution_{fc}",
...
core_knowledge_points = st.text_input(
    ...
    key=f"core_knowledge_points_{fc}"
)
if existing_kps:
    with st.expander(f"📚 已录知识点（{len(existing_kps)}个，点击可补充）", expanded=False):
        ...
        for i, kp in enumerate(existing_kps):
            with cols[i % 3]:
                if st.button(kp, key=f"kp_{kp}_{fc}"):
                    current = st.session_state.get(f"core_knowledge_points_{fc}", "") or ""
                    if kp not in current:
                        new_val = f"{current}, {kp}" if current else kp
                        st.session_state[f"core_knowledge_points_{fc}"] = new_val
                        st.rerun()

key_insight = st.text_input(
    "💡 一句话大彻大悟",
    placeholder="例如：看到有界闭区间上连续函数，先想介值定理和零点定理",
    key=f"key_insight_{fc}"
)
...
if st.button("💾 保存错题", type="primary", use_container_width=True):
    ...
    if errors:
        ...
        st.session_state._form_counter += 1
        st.rerun()
